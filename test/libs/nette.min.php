...
/**
 * Nette Framework (version 2.0-dev released on 2011-02-07, http://nette.org)
 *
...
constructUrl(PresenterRequest$appRequest,Uri$refUri);}interface
IFreezable{function
freeze();function
isFrozen();}interface
...
FILLED=':filled';const
VALID=':valid';const
PROTECTION='HiddenField::validateEqual';const
...
addProtection($message=NULL,$timeout=NULL){$session=$this->getSession()->getNamespace('Nette.Forms.Form/CSRF');$key="key$timeout";if(isset($session->$key)){$token=$session->$key;}else{$session->$key=$token=String::random();}$session->setExpiration($timeout,$key);$this[self::PROTECTOR_ID]=new
...
function
receiveHttpData(){$httpRequest=$this->getHttpRequest();if(strcasecmp($this->getMethod(),$httpRequest->getMethod())){return;}if($httpRequest->isMethod('post')){$data=ArrayTools::mergeTree($httpRequest->getPost(),$httpRequest->getFiles());}else{$data=$httpRequest->getQuery();}if($tracker=$this->getComponent(self::TRACKER_ID,FALSE)){if(!isset($data[self::TRACKER_ID])||$data[self::TRACKER_ID]!==$tracker->getValue()){return;}}return$data;}function
...
new
ApplicationException('Too many loops detected in application life cycle.');}if(!$request){$this->onStartup($this);$session=$this->getSession();if(!$session->isStarted()&&$session->exists()){$session->start();}$router=$this->getRouter();Debug::addPanel(new
...
storeRequest($expiration='+ 10 minutes'){$session=$this->getSession('Nette.Application/requests');do{$key=String::random(5);}while(isset($session[$key]));$session[$key]=end($this->requests);$session->setExpiration($expiration,$key);return$key;}function
...
getFlashSession(){if(empty($this->params[self::FLASH_KEY])){$this->params[self::FLASH_KEY]=String::random(4);}return$this->getSession('Nette.Application.Flash/'.$this->params[self::FLASH_KEY]);}protected
function
getHttpRequest(){return
Environment::getHttpRequest();}protected
function
...
new
InvalidPresenterException("Cannot load presenter '$name', class '$class' is not IPresenter implementor.");}if($reflection->isAbstract()){throw
new
...
Json::encode($this->payload);}}class
RedirectingResponse
extends
Object
implements
...
constructUrl(PresenterRequest$appRequest,Uri$refUri){return
NULL;}function
...
NULL;}function
constructUrl(PresenterRequest$appRequest,Uri$refUri){if($this->cachedRoutes===NULL){$routes=array();$routes['*']=array();foreach($this
...
constructUrl(PresenterRequest$appRequest,Uri$refUri){if($this->flags&self::ONE_WAY){return
...
new
PresenterRequest($presenter,$httpRequest->getMethod(),$params,$httpRequest->getPost(),$httpRequest->getFiles(),array(PresenterRequest::SECURED=>$httpRequest->isSecured()));}function
constructUrl(PresenterRequest$appRequest,Uri$refUri){$params=$appRequest->getParams();$presenter=$appRequest->getPresenterName();if(strncasecmp($presenter,$this->module,strlen($this->module))===0){$params[self::PRESENTER_KEY]=substr($presenter,strlen($this->module));}else{return
...
Memcache;Debug::tryError();$this->memcache->connect($host,$port);if(Debug::catchError($e)){throw
new
...
static$extensions=array('ini'=>'ConfigAdapterIni','neon'=>'ConfigAdapterNeon');static
function
...
new
FileNotFoundException("File '$file' is missing or is not readable.");}Debug::tryError();$ini=parse_ini_file($file,TRUE);if(Debug::catchError($e)){throw$e;}$separator=trim(self::$sectionSeparator);$data=array();foreach($ini
...
new
InvalidStateException("Invalid section [$secName] in '$file'.");}}}else{$cursor=&$data[$secName];}if(is_array($secData)&&is_array($cursor)){$secData=ArrayTools::mergeTree($secData,$cursor);}$cursor=$secData;}return$data;}static
function
...
load($file){if(!is_file($file)||!is_readable($file)){throw
new
FileNotFoundException("File '$file' is missing or is not readable.");}$neon=Neon::decode(file_get_contents($file));$separator=trim(self::$sectionSeparator);$data=array();foreach($neon
...
__construct($dsn,$username=NULL,$password=NULL,array$options=NULL){parent::__construct($dsn,$username,$password,$options);$this->setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);$this->setAttribute(PDO::ATTR_STATEMENT_CLASS,array('Statement',array($this)));$class='Pdo'.$this->getAttribute(PDO::ATTR_DRIVER_NAME).'Driver';if(class_exists($class)){$this->driver=new$class($this,(array)$options);}$this->preprocessor=new
SqlPreprocessor($this);$this->databaseReflection=new
DatabaseReflection;if(!Debug::$productionMode){Debug::addPanel($panel=new
...
table($table){return
new
TableSelection($table,$this);}function
...
logQuery(Statement$result,array$params=NULL){$source=NULL;foreach(debug_backtrace(FALSE)as$row){if(isset($row['file'])&&is_file($row['file'])&&strpos($row['file'],NETTE_DIR.DIRECTORY_SEPARATOR)!==0){$source=array($row['file'],(int)$row['line']);break;}}$this->totalTime+=$result->time;$this->queries[]=array($result->queryString,$params,$result->time,$result->rowCount(),$result->getConnection(),$source);}function
...
as$i=>$query){list($sql,$params,$time,$rows,$connection,$source)=$query;$explain=NULL;if($this->explain&&preg_match('#\s*SELECT\s#iA',$sql)){try{$explain=$connection->queryArgs('EXPLAIN '.$sql,$params)->fetchAll();}catch(PDOException$e){}}$s.='<tr><td>'.sprintf('%0.3f',$time*1000);if($explain){$s.="<br /><a href='#' class='nette-toggler' rel='#nette-debug-database-row-{$h($this->name)}-$i'>explain&nbsp;&#x25ba;</a>";}$s.='</td><td class="database-sql">'.Connection::highlightSql(String::truncate($sql,self::$maxLength));if($explain){$s.="<table id='nette-debug-database-row-{$h($this->name)}-$i' class='nette-collapsed'><tr>";foreach($explain[0]as$col=>$foo){$s.="<th>{$h($col)}</th>";}$s.="</tr>";foreach($explain
as$row){$s.="<tr>";foreach($row
as$col){$s.="<td>{$h($col)}</td>";}$s.="</tr>";}$s.="</table>";}if($source){list($file,$line)=$source;$s.=(Debug::$editor?"<a href='{$h(DebugHelpers::editorLink($file,$line))}'":'<span')." class='database-source' title='{$h($file)}:$line'>"."{$h(basename(dirname($file)).'/'.basename($file))}:$line".(Debug::$editor?'</a>':'</span>');}$s.='</td><td>';foreach($params
as$param){$s.="{$h(String::truncate($param,self::$maxLength))}<br>";}$s.='</td><td>'.$rows.'</td></tr>';}return
...
__construct(Connection$connection,array$options){$this->connection=$connection;}function
...
implements
ISupplementalDriver{private$connection;function
__construct(Connection$connection,array$options){$this->connection=$connection;$charset=isset($options['charset'])?$options['charset']:'utf8';if($charset){$connection->exec("SET NAMES '$charset'");}if(isset($options['sqlmode'])){$connection->exec("SET sql_mode='$options[sqlmode]'");}$connection->exec("SET time_zone='".date('P')."'");}function
...
__construct(Connection$connection,array$options){$this->connection=$connection;$this->fmtDateTime=isset($options['formatDateTime'])?$options['formatDateTime']:'U';}function
delimite($name){return'"'.str_replace('"','""',$name).'"';}function
...
DatabaseReflection
extends
Object{private$primary;private$foreign;private$table;function
...
__construct($table,Connection$connection){$this->name=$table;$this->connection=$connection;$this->primary=$this->getPrimary($table);$this->delimitedName=$connection->getSupplementalDriver()->delimite($this->name);$this->delimitedPrimary=$connection->getSupplementalDriver()->delimite($this->primary);}function
...
SqlPreprocessor
extends
Object{private$connection;private$driver;private$params;private$remaining;private$counter;private$arrayMode;function
__construct(Connection$connection){$this->connection=$connection;$this->driver=$connection->getSupplementalDriver();}function
process($sql,$params){$this->params=$params;$this->counter=0;$this->remaining=array();$cmd=strtoupper(substr(ltrim($sql),0,6));$this->arrayMode=$cmd==='INSERT'||$cmd==='REPLAC'?'values':'assoc';$sql=String::replace($sql,'~\'.*?\'|".*?"|:[a-zA-Z0-9_]+:|\?~s',array($this,'callback'));while($this->counter<count($params)){$sql.=' '.$this->formatValue($params[$this->counter++]);}return
...
instanceof
SqlLiteral){return$value->value;}else{$this->remaining[]=$value;return'?';}}}if(class_exists('PDO')){class
Statement
extends
...
TRUE;}}Debug::_init();class
Configurator
extends
Object{public$defaultConfigFile='%appDir%/config.neon';public$defaultServices=array('Nette\\Application\\Application'=>array(__CLASS__,'createApplication'),'Nette\\Web\\HttpContext'=>'HttpContext','Nette\\Web\\IHttpRequest'=>array(__CLASS__,'createHttpRequest'),'Nette\\Web\\IHttpResponse'=>'HttpResponse','Nette\\Web\\IUser'=>'User','Nette\\Caching\\ICacheStorage'=>array(__CLASS__,'createCacheStorage'),'Nette\\Caching\\ICacheJournal'=>array(__CLASS__,'createCacheJournal'),'Nette\\Mail\\IMailer'=>array(__CLASS__,'createMailer'),'Nette\\Web\\Session'=>'Session','Nette\\Loaders\\RobotLoader'=>array(__CLASS__,'createRobotLoader'));function
...
Environment::getContext();$context->addService('Nette\\Application\\IRouter','MultiRouter');if(!$context->hasService('Nette\\Application\\IPresenterLoader')){$context->addService('Nette\\Application\\IPresenterLoader',callback(create_function('','
				return new PresenterLoader(Environment::getVariable(\'appDir\'));
			')));}$class=isset($options['class'])?$options['class']:'Application';$application=new$class;$application->setContext($context);$application->catchExceptions=Environment::isProduction();return$application;}static
function
createHttpRequest(){$factory=new
HttpRequestFactory;$factory->setEncoding('UTF-8');return$factory->createHttpRequest();}static
function
createCacheStorage(){$dir=Environment::getVariable('tempDir').'/cache';umask(0000);@mkdir($dir,0777);return
new
FileStorage($dir,Environment::getService('Nette\\Caching\\ICacheJournal'));}static
function
createCacheJournal(){return
new
FileJournal(Environment::getVariable('tempDir'));}static
function
createMailer(array$options=NULL){if(isset($options['smtp'])){return
new
SmtpMailer($options);}else{return
new
SendmailMailer;}}static
function
createRobotLoader(array$options=NULL){$loader=new
RobotLoader;$loader->autoRebuild=isset($options['autoRebuild'])?$options['autoRebuild']:!Environment::isProduction();$loader->setCacheStorage(Environment::getService('Nette\\Caching\\ICacheStorage'));if(isset($options['directory'])){$loader->addDirectory($options['directory']);}else{foreach(array('appDir','libsDir')as$var){if($dir=Environment::getVariable($var,NULL)){$loader->addDirectory($dir);}}}$loader->register();return$loader;}}final
class
Environment{const
...
new
MemberAccessException("Call to undefined static method Environment::$name().");}}static
function
getHttpRequest(){return
...
getForm($need=TRUE){return$this->lookup('Form',$need);}function
getHtmlName(){if($this->htmlName===NULL){$s='';$name=$this->getName();$obj=$this->lookup('FormContainer',TRUE);while(!$obj
instanceof
Form){$s="[$name]$s";$name=$obj->getName();$obj=$obj->lookup('FormContainer',TRUE);}$name.=$s;if(is_numeric($name)||in_array($name,array('attributes','children','elements','focus','length','reset','style','submit','onsubmit'))){$name.='_';}$this->htmlName=$name;}return$this->htmlName;}function
...
getControl(){$control=parent::getControl();if($this->skipFirst){reset($this->items);$control->data('nette-empty-value',$this->useKeys?key($this->items):current($this->items));}$selected=$this->getValue();$selected=is_array($selected)?array_flip($selected):array($selected=>TRUE);$option=Html::el('option');foreach($this->items
...
instanceof
Html){$dest->add((string)$value2->selected(isset($selected[$key2])));}else{$key2=$this->useKeys?$key2:$value2;$value2=$this->translate($value2);$dest->add((string)$option->value($key2===$value2?NULL:$key2)->selected(isset($selected[$key2]))->setText($value2));}}}return$control;}}class
MultiSelectBox
extends
SelectBox{function
...
function
validatePattern(TextBase$control,$pattern){return(bool)String::match($control->getValue(),"{^($pattern)$}u");}static
function
...
sanitize($value){if($this->control->maxlength&&String::length($value)>$this->control->maxlength){$value=iconv_substr($value,0,$this->control->maxlength,'UTF-8');}return
String::trim(strtr($value,"\r\n",'  '));}function
...
RobotLoader
extends
AutoLoader{public$scanDirs=array();public$ignoreDirs='.*, *.old, *.bak, *.tmp, temp';public$acceptFiles='*.php, *.php5';public$autoRebuild=TRUE;private$list=array();private$files;private$rebuilt=FALSE;private$cacheStorage;function
...
InvalidStateException("Ambiguous class '$class' resolution; defined in $file and in ".$this->list[$lClass][0].".");if(PHP_VERSION_ID<50300){Debug::_exceptionHandler($e);exit;}else{throw$e;}}$this->list[$lClass]=array($file,$time,$class);$this->files[$file]=$time;}private
function
scanDirectory($dir){if(is_dir($dir)){$disallow=array();$iterator=Finder::findFiles(String::split($this->acceptFiles,'#[,\s]+#'))->filter(create_function('$file','extract(NClosureFix::$vars['.NClosureFix::uses(array('disallow'=>&$disallow)).'], EXTR_REFS);
					return !isset($disallow[$file->getPathname()]);
				'))->from($dir)->exclude(String::split($this->ignoreDirs,'#[,\s]+#'))->filter($filter=create_function('$dir','extract(NClosureFix::$vars['.NClosureFix::uses(array('disallow'=>&$disallow)).'], EXTR_REFS);
...
new
InvalidArgumentException("Name must not contain line separator.");}$tmp[$email]=$name;}}else{$value=(string)$value;if(!String::checkEncoding($value)){throw
new
...
generateMessage(){$output='';$boundary='--------'.String::random();foreach($this->headers
...
new
FileNotFoundException("Unable to read file '$file'.");}}else{$content=(string)$content;}$part->setBody($content);$part->setContentType($contentType?$contentType:Tools::detectMimeTypeFromString($content));$part->setEncoding(preg_match('#(multipart|message)/#A',$contentType)?self::ENCODING_8BIT:self::ENCODING_BASE64);$part->setHeader('Content-Disposition',$disposition.'; filename="'.String::fixEncoding(basename($file)).'"');return$part;}function
...
FunctionReflection($key);}return$res;}function
getReflection(){return
new
ClassReflection($this);}function
__call($name,$args){return
...
ParameterReflection($this->value,$val->getName());}return$res;}function
getReflection(){return
new
ClassReflection($this);}function
__call($name,$args){return
...
getClassName(){return($tmp=String::match($this,'#>\s+([a-z0-9_\\\\]+)#i'))?$tmp[1]:NULL;}function
...
FunctionReflection($this->function);}function
getReflection(){return
new
ClassReflection($this);}function
__call($name,$args){return
...
ob_get_clean();}catch(Exception$e){ob_end_clean();if(func_num_args()&&func_get_arg(0)){throw$e;}else{Debug::toStringException($e);}}}function
...
instanceof
TemplateCacheStorage){$storage->hint=str_replace(dirname(dirname($this->file)),'',$this->file);}$cached=$content=$cache[$this->file];if($content===NULL){try{$content=$this->compile(file_get_contents($this->file));$content="<?php\n\n// source file: $this->file\n\n?>$content";}catch(TemplateException$e){$e->setSourceFile($this->file);throw$e;}$cache->save($this->file,$content,array(Cache::FILES=>$this->file,Cache::CONSTS=>'Framework::REVISION'));$cache->release();$cached=$cache[$this->file];}if($cached!==NULL&&$storage
instanceof
...
getPanel(){$link=DebugHelpers::editorLink($this->sourceFile,$this->sourceLine);return'<p><b>File:</b> '.($link?'<a href="'.htmlspecialchars($link).'">':'').htmlspecialchars($this->sourceFile).($link?'</a>':'').'&nbsp; <b>Line:</b> '.($this->sourceLine?$this->sourceLine:'n/a').'</p>'.($this->sourceLine?'<pre>'.DebugHelpers::highlightFile($this->sourceFile,$this->sourceLine).'</pre>':'');}function
getId(){}}class
LatteException
extends
...
contextCData(){$tag=end($this->tags);$matches=$this->match('~
			...
			'.$this->macroRe.'           ##  curly tag
		~xsi');if($matches&&empty($matches['macro'])&&empty($matches['comment'])){$tag->closing=TRUE;$tag->pos=strlen($this->output);$this->context=self::CONTEXT_TAG;$this->escape='TemplateHelpers::escapeHtml';}return$matches;}private
function
...
Tokenizer(array(self::T_WHITESPACE=>'\s+',self::T_COMMENT=>'(?s)/\*.*?\*/',LatteFilter::RE_STRING,'(?:true|false|null|and|or|xor|clone|new|instanceof|return|continue|break|[A-Z_][A-Z0-9_]{2,})(?!\w)','\([a-z]+\)',self::T_VARIABLE=>'\$\w+',self::T_NUMBER=>'[+-]?[0-9]+(?:\.[0-9]+)?(?:e[0-9]+)?',self::T_SYMBOL=>'\w+(?:-\w+)*','::|=>|[^"\']'),'u');}function
initialize($filter,&$s){$this->filter=$filter;$this->nodes=array();$this->blocks=array();$this->namedBlocks=array();$this->extends=NULL;$this->uniq=String::random();$this->cacheCounter=0;$filter->context=LatteFilter::CONTEXT_TEXT;$filter->escape='TemplateHelpers::escapeHtml';}function
...
String::replace($this->macros[$macro],'#%(.*?)%#',callback(create_function('$m','extract(NClosureFix::$vars['.NClosureFix::uses(array('This'=>$This,'node'=>$node)).'], EXTR_REFS);
				if ($m[1]) {
...
new
LatteException("Cannot include $destination block outside of any block.",0,$this->filter->line);}$destination=$item[1];}$name=var_export($destination,TRUE);$params.=$isDefinition?'get_defined_vars()':'$template->getParams()';$cmd=isset($this->namedBlocks[$destination])&&!$parent?"call_user_func(reset(\$_l->blocks[$name]), \$_l, $params)":'LatteMacros::callBlock'.($parent?'Parent':'')."(\$_l, $name, $params)";return$modifiers?"ob_start(); $cmd; echo ".$this->formatModifiers('ob_get_clean()',$modifiers):$cmd;}else{$destination=$this->formatString($destination);$cmd='LatteMacros::includeTemplate('.$destination.', '.$params.'$template->getParams(), $_l->templates['.var_export($this->uniq,TRUE).'])';return$modifiers?'echo '.$this->formatModifiers($cmd.'->__toString(TRUE)',$modifiers):$cmd.'->render()';}}function
...
macroCache($content){return'if (CachingHelper::create('.var_export($this->uniq.':'.$this->cacheCounter++,TRUE).', $_l->g->caches'.$this->formatArray($content,', ').')) {';}function
macroForeach($content){return'$iterator = $_l->its[] = new SmartCachingIterator('.preg_replace('#(.*)\s+as\s+#i','$1) as ',$this->formatMacroArgs($content),1);}function
...
macroDump($content){return'Debug::barDump('.($content?'array('.var_export($this->formatMacroArgs($content),TRUE)." => $content)":'get_defined_vars()').', "Template " . str_replace(dirname(dirname($template->getFile())), "\xE2\x80\xA6", $template->getFile()))';}function
...
macroIfCurrent($content){return($content?'try { $presenter->link('.$this->formatLink($content).'); } catch (InvalidLinkException $e) {}':'').'; if ($presenter->getLastCreatedRequestFlag("current")):';}private
function
...
instanceof
Control){foreach($control->getComponents(FALSE,'Control')as$child){if($child->isControlInvalid()){$child->render();}}}}}final
class
...
function
netteLinks($s){return
String::replace($s,'#(src|href|action)\s*=\s*(["\'])(nette:.*?)([\#"\'])#',callback(create_function('$m','
...
String::replace($s,'#<texy([^>]*)>(.*?)</texy>#s',callback(create_function('$m','
...
str_replace(']]>',']]\x3E',Json::encode($s));}static
function
escapeHtmlJs($s){return
htmlSpecialChars(self::escapeJs($s),ENT_QUOTES);}static
function
strip($s){return
String::replace($s,'#(</textarea|</pre|</script|^).*?(?=<textarea|<pre|<script|$)#si',callback(create_function('$m','
				return trim(preg_replace("#[ \\t\\r\\n]+#", " ", $m[0]));
			')));}static
function
indent($s,$level=1,$chars="\t"){if($level>=1){$s=String::replace($s,'#<(textarea|pre).*?</\\1#si',callback(create_function('$m','
				return strtr($m[0], " \\t\\r\\n", "\\x1F\\x1E\\x1D\\x1A");
			')));$s=String::indent($s,$level,$chars);$s=strtr($s,"\x1F\x1E\x1D\x1A"," \t\r\n");}return$s;}static
function
date($time,$format=NULL){if($time==NULL){return
NULL;}if(!isset($format)){$format=self::$dateFormat;}$time=Tools::createDateTime($time);return
...
function
dataStream($data,$type=NULL){if($type===NULL){$type=Tools::detectMimeTypeFromString($data,NULL);}return'data:'.($type?"$type;":'').'base64,'.base64_encode($data);}static
function
...
function
getCacheFile($key){$key=substr_replace($key,trim(strtr($this->hint,'\\/@','.._'),'.').'-',strpos($key,Cache::NAMESPACE_SEPARATOR)+1,0);return
...
RecursiveCallbackFilterIterator($iterator,create_function('$file','extract(NClosureFix::$vars['.NClosureFix::uses(array('filters'=>$filters)).'], EXTR_REFS);
				if (!$file->isFile()) {
...
CallbackFilterIterator($iterator,create_function('$file','extract(NClosureFix::$vars['.NClosureFix::uses(array('groups'=>$groups)).'], EXTR_REFS);
...
new
InvalidArgumentException('Invalid date predicate format.');}list(,$operator,$date)=$matches;$operator=$operator?$operator:'=';}$date=Tools::createDateTime($date)->format('U');return$this->filter(create_function('$file','extract(NClosureFix::$vars['.NClosureFix::uses(array('operator'=>$operator,'date'=>$date)).'], EXTR_REFS);
...
REVISION='421651c released on 2011-02-07';public
...
static$brackets=array('['=>']','{'=>'}','('=>')');private$n=0;static
function
encode($var,$options=NULL){if(is_object($var)){$obj=$var;$var=array();foreach($obj
as$k=>$v){$var[$k]=$v;}}if(is_array($var)){$isArray=array_keys($var)===range(0,count($var)-1);$s='';if($options&self::BLOCK){foreach($var
as$k=>$v){$s.=$isArray?'- ':self::encode($k).':';if(is_object($v)||is_array($v)){$c=0;foreach($v
as$k2=>$v2){if(is_array($v2)||is_object($v2)||(string)$k2!==(string)$c++){$s.="\n\t".str_replace("\n","\n\t",self::encode($v,self::BLOCK))."\n";continue
2;}}}$s.=(!$isArray&&$v===NULL)?"\n":' '.self::encode($v)."\n";}return$s;}else{foreach($var
as$k=>$v){$s.=($isArray?'':self::encode($k).': ').self::encode($v).', ';}return($isArray?'[':'{').substr($s,0,-2).($isArray?']':'}');}}elseif(is_string($var)&&!is_numeric($var)&&!preg_match('~[\x00-\x1F]|^(true|false|yes|no|null)$~i',$var)&&preg_match('~^'.self::$patterns[5].'$~',$var)){return$var;}else{return
...
new
InvalidStateException("Callback '$textual' is not callable.");}$res=preg_replace_callback($pattern,$replacement,$subject,$limit);if(Debug::catchError($e)){$trace=$e->getTrace();if(isset($trace[2]['class'])&&$trace[2]['class']===__CLASS__){throw
new
...
getContentType(){if($this->isOk()&&$this->type===NULL){$this->type=Tools::detectMimeType($this->tmpName);}return$this->type;}function
...
new
InvalidStateException('A session had already been started by session.auto-start or session_start().');}$this->configure($this->options);Debug::tryError();session_start();if(Debug::catchError($e)){@session_write_close();throw
new
InvalidStateException($e->getMessage());}self::$started=TRUE;if($this->regenerationNeeded){session_regenerate_id(TRUE);$this->regenerationNeeded=FALSE;}unset($_SESSION['__NT'],$_SESSION['__NS'],$_SESSION['__NM']);$nf=&$_SESSION['__NF'];if(empty($nf)){$nf=array('C'=>0);}else{$nf['C']++;}$browserKey=$this->getHttpRequest()->getCookie('nette-browser');if(!$browserKey){$browserKey=String::random();}$browserClosed=!isset($nf['B'])||$nf['B']!==$browserKey;$nf['B']=$browserKey;$this->sendCookie();if(isset($nf['META'])){$now=time();foreach($nf['META']as$namespace=>$metadata){if(is_array($metadata)){foreach($metadata
...
new
InvalidStateException("Unable to set '$key' when session has been started.");}$key="session_$key";$key($value);}elseif(strncmp($key,'cookie_',7)===0){if(!isset($cookie)){$cookie=session_get_cookie_params();}$cookie[substr($key,7)]=$value;}elseif(!function_exists('ini_set')){if(ini_get($key)!=$value&&!Framework::$iAmUsingBadHost){throw
new
...
setExpiration($time){if(empty($time)){return$this->setOptions(array('gc_maxlifetime'=>self::DEFAULT_FILE_LIFETIME,'cookie_lifetime'=>0));}else{$time=Tools::createDateTime($time)->format('U')-time();return$this->setOptions(array('gc_maxlifetime'=>$time,'cookie_lifetime'=>$time));}}function
