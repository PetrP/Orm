...
?>' . $s;
		...
		$s = "<?php\n"
			. '$_l = LatteMacros::initRuntime($template, '
	...
	public function macroInclude($content, $modifiers, $isDefinition = FALSE)
	{
		...
		if ($destination === NULL) {
			...
			$cmd = 'LatteMacros::includeTemplate(' . $destination . ', '
	...
	{
		return '$iterator = $_l->its[] = new SmartCachingIterator('
	...
	public function macroContentType($content)
	{
		...
		// temporary solution
		if (strpos($content, '/')) {
			return 'Environment::getHttpResponse()->setHeader("Content-Type", "' . $content . '")';
		}
	}
